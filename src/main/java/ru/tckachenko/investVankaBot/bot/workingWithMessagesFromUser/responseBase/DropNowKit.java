...
public class DropNowKit extends AnswerWithTextOnly {
    ...
    public String getMessage() {
        ...
        message += gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .sorted(new TiketInformationComparatorToDropNowKit()).limit(5)
        ...
        message += "\nСреди акции категории BRONZE в лидерах падения за день, на текущее время, находятся:\n" + gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .skip(10)
                .limit(25)
                .sorted(new TiketInformationComparatorToDropNowKit())
                .limit(10)
                .filter(s->s.getChangesday()<0.0)
                .map((p)->(numb++)+". "+p.getTiket() +" ("+ p.getChangesday()+"%)\n").collect(Collectors.joining());
        numb = 1;
        message += "\nСреди акции категории SILVER в лидерах падения за день, на текущее время, находятся:\n" + gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .skip(25)
                .limit(50)
                .sorted(new TiketInformationComparatorToDropNowKit()).limit(10)
                .filter(s->s.getChangesday()<0.0)
                .map((p)->(numb++)+". "+p.getTiket() +" ("+ p.getChangesday()+"%)\n").collect(Collectors.joining());
        numb = 1;
