...
import ru.tckachenko.investVankaBot.dataProcessor.comparators.TiketInformationComparatorToGrowingYearKit;
...
public class GrowingYearKit extends AnswerWithTextOnly {
    ...
    public String getMessage() {
        numb = 1;
        message = "Среди акции категории GOLD в лидерах роста, за год, находятся:\n";
        message += gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .sorted(new TiketInformationComparatorToGrowingYearKit()).limit(5)
        ...
        message += "\nСреди акции категории BRONZE в лидерах роста, за год, находятся:\n" + gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .skip(10)
                .limit(25)
                .sorted(new TiketInformationComparatorToGrowingYearKit())
                .limit(10)
                .filter(s->s.getChangesyear()>0.0)
                .map((p)->(numb++)+". "+p.getTiket() +" (+"+ p.getChangesyear()+"%)\n").collect(Collectors.joining());
        numb = 1;
        message += "\nСреди акции категории SILVER в лидерах роста, за год, находятся:\n" + gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .skip(25)
                .limit(50)
                .sorted(new TiketInformationComparatorToGrowingYearKit()).limit(10)
                .filter(s->s.getChangesyear()>0.0)
                .map((p)->(numb++)+". "+p.getTiket() +" (+"+ p.getChangesyear()+"%)\n").collect(Collectors.joining());
        numb = 1;
