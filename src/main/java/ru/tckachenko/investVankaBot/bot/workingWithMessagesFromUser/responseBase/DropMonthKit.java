...
public class DropMonthKit extends AnswerWithTextOnly {
    ...
    public String getMessage() {
        ...
        message += gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .sorted(new TiketInformationComparatorToDropMonthKit()).limit(5)
        ...
        message += "\nСреди акции категории BRONZE в лидерах падения за месяц, находятся:\n" + gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .skip(10)
                .limit(25)
                .sorted(new TiketInformationComparatorToDropMonthKit())
                .limit(10)
                .filter(s -> s.getChangesmonth() < 0.0)
                .map((p) -> (numb++) + ". " + p.getTiket() + " (" + p.getChangesmonth() + "%)\n").collect(Collectors.joining());
        numb = 1;
        message += "\nСреди акции категории SILVER в лидерах падения за месяц, находятся:\n" + gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .skip(25)
                .limit(50)
                .sorted(new TiketInformationComparatorToDropMonthKit()).limit(10)
                .filter(s -> s.getChangesmonth() < 0.0)
                .map((p) -> (numb++) + ". " + p.getTiket() + " (" + p.getChangesmonth() + "%)\n").collect(Collectors.joining());
        numb = 1;
