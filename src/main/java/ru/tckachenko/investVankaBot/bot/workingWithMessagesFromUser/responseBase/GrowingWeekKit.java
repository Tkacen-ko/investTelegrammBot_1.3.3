...
public class GrowingWeekKit extends AnswerWithTextOnly {
    ...
    public String getMessage() {
        numb = 1;
        message = "Среди акции категории GOLD в лидерах роста, за неделю, находятся:\n";
        message += gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .sorted(new TiketInformationComparatorToGrowingWeekKit()).limit(5)
        ...
        message += "\nСреди акции категории BRONZE в лидерах роста, за неделю, находятся:\n" + gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .skip(10)
                .limit(25)
                .sorted(new TiketInformationComparatorToGrowingWeekKit())
                .limit(10)
                .filter(s->s.getChangesweek()>0.0)
                .map((p)->(numb++)+". "+p.getTiket() +" (+"+ p.getChangesweek()+"%)\n").collect(Collectors.joining());
        numb = 1;
        message += "\nСреди акции категории SILVER в лидерах роста, за неделю, находятся:\n" + gettingDataFromDatabase.getDataToTikerRealTime()
                ...
                .skip(25)
                .limit(50)
                .sorted(new TiketInformationComparatorToGrowingWeekKit()).limit(10)
                .filter(s->s.getChangesweek()>0.0)
                .map((p)->(numb++)+". "+p.getTiket() +" (+"+ p.getChangesweek()+"%)\n").collect(Collectors.joining());
        numb = 1;
