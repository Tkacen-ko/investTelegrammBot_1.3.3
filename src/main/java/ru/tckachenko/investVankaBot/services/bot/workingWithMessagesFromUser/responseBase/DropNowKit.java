...
import ru.tckachenko.investVankaBot.dao.helperСlassesWorkingData.comparators.TikerInformationComparatorToDropNowKit;
...
@Component
public class DropNowKit extends AnswerWithTextOnly {
    ...
    @Autowired
    public DropNowKit(GettingDataFromDatabase gettingDataFromDatabase) {
    ...
    public String getMessage() {
        numb = 1;
        message = "Среди акции категории GOLD\uD83E\uDD47 в лидерах падения за день, на текущее время, находятся:\n";
        message += gettingDataFromDatabase.getDataToTickerRealTime()
                ...
                .sorted(new TikerInformationComparatorToDropNowKit()).limit(5)
                .filter(s -> s.getChangesday() < 0.0)
                .map((p) -> (numb++) + ". " + p.getTicker() + " (" + p.getChangesday() + "%)\n").collect(Collectors.joining());
        numb = 1;
        message += "\nСреди акции категории SILVER\uD83E\uDD48 в лидерах падения за день, на текущее время, находятся:\n" + gettingDataFromDatabase.getDataToTickerRealTime()
                .stream()
                .filter(s -> s.getPrice() != 0.0)
                .sorted(new TikerInformationComparatorToCapitalizationmlrdru())
                .skip(10)
                .limit(25)
                .sorted(new TikerInformationComparatorToDropNowKit())
                .limit(10)
                .filter(s -> s.getChangesday() < 0.0)
                .map((p) -> (numb++) + ". " + p.getTicker() + " (" + p.getChangesday() + "%)\n").collect(Collectors.joining());
        numb = 1;
        message += "\nСреди акции категории BRONZE\uD83E\uDD49 в лидерах падения за день, на текущее время, находятся:\n" + gettingDataFromDatabase.getDataToTickerRealTime()
                .stream()
                .filter(s -> s.getPrice() != 0.0)
                .sorted(new TikerInformationComparatorToCapitalizationmlrdru())
                .skip(25)
                .limit(50)
                .sorted(new TikerInformationComparatorToDropNowKit()).limit(10)
                .filter(s -> s.getChangesday() < 0.0)
                .map((p) -> (numb++) + ". " + p.getTicker() + " (" + p.getChangesday() + "%)\n").collect(Collectors.joining());
        numb = 1;
        message += "\nСреди акции категории DNO\uD83C\uDFA2 в лидерах падения за день, на текущее время, находятся:\n" + gettingDataFromDatabase.getDataToTickerRealTime()
                .stream()
                .filter(s -> s.getPrice() != 0.0)
                .sorted(new TikerInformationComparatorToCapitalizationmlrdru())
                .skip(50)
